...
let env checkpoint =
  ...
  | Mparser.MenhirInterpreter.HandlingError env -> env
  | _ -> assert false
...
let state checkpoint : int =
  match Mparser.MenhirInterpreter.top (env checkpoint) with
  | Some (Mparser.MenhirInterpreter.Element (s, _, _, _)) ->
      let result = Mparser.MenhirInterpreter.number s in
      Format.eprintf "state : %d@." result;
      result
  | None ->
...
let get text checkpoint i =
  match Mparser.MenhirInterpreter.get i (env checkpoint) with
  | Some (Mparser.MenhirInterpreter.Element (_, _, pos1, pos2)) ->
      show text (pos1, pos2)
  | None ->
...
let fail text buffer (checkpoint : _ Mparser.MenhirInterpreter.checkpoint) =
  (* Fetch an error message from the database. *)
  let message = MParserMessages.message (state checkpoint) in
  ...
  Errors.raise_spanned_error indication
    (Parse_utils.mk_position (MenhirLib.ErrorReports.last buffer))

let parse_file (file_name : string) (* : Irj_ast.irj_file *) =
  let text, filebuf =
    try MenhirLib.LexerUtil.read file_name
    with Sys_error msg ->
      Errors.raise_error
        (Format.asprintf "Unable to open file %s (%s)" file_name msg)
  in
  let supplier =
    Mparser.MenhirInterpreter.lexer_lexbuf_to_supplier Mlexer.token filebuf
  in
  ...
  let checkpoint = Mparser.Incremental.source_file filebuf.lex_curr_p in
  (* Run the parser. *)
  (* We do not handle [Lexer.Error] because we know that we will not
     encounter a lexical error during this second parsing run. *)
  Mparser.MenhirInterpreter.loop_handle succeed (fail text buffer) supplier
    checkpoint
